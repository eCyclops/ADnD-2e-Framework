...
[h, foreach(thisWeapon,myWeaponsSorted,""), code :{
    ...
    [h, if(monkDamage), code: {
      [h: monkDamage = monkWeaponDamage]
      [h: monkDamageRow = "<tr><td>Monk</td><td>"+monkDamage+"</td></tr>"]
    };
    {
      [h: monkDamage = 0]
      [h: monkDamageRow = ""]
		...
		[h: totalDmgMod = toDamage+strDamage+profDamage+raceDamage+monkDamage+otherDamage]
		...
		[h: initLink = macroLink(speed, 'DO_Initiative@Lib:Initiative','none',strformat("myID=%{myID}; initiativeAction=Attack with %{name}; initiativeModifier=%{speed};"),myID)]
...
[h: macro.return = outTxt][h: myID = getStrProp(arg(0),"myID")]
